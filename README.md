...
- 📈 鼠标悬停提示：每个插座显示当前功率及10分钟/1小时/24小时最大/最小/平均（需开启历史记录）
...
<插件配置目录>\
  ...
  MijiaPower_history_1.json ← 第1个插座的功率历史（如果启用）
  MijiaPower_history_2.json ← 第2个插座的功率历史
  ...
```
...
> 💡 **从 v1.0 升级**：旧版单设备配置（`[Device]` 段）会被自动迁移为 `[Device1]`，历史文件 `MijiaPower_history.json` 会被自动迁移为 `MijiaPower_history_1.json`，无需手动修改。
